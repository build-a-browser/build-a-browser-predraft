...
public class GridTrackSizeParser implements PropertyValueParser {
  ...
  public CSSValue parseTrackSize(CSSTokenStream stream) throws IOException {
    if (
  ...
  public CSSValue parseFixedSize(CSSTokenStream stream) throws IOException {
    if (
      stream.peek() instanceof FunctionValue functionValue
      && functionValue.name().equals("minmax")
    ) {
  ...
  private CSSValue parseTrackBreadth(CSSTokenStream stream) throws IOException {
    return parseTrackBreadthShared(stream, sizeParserTrack, true);
  }

  private CSSValue parseInflexibleBreadth(CSSTokenStream stream) throws IOException {
    return parseTrackBreadthShared(stream, sizeParserTrack, false);
  }

  private CSSValue parseFixedBreadth(CSSTokenStream stream) throws IOException {
    return parseTrackBreadthShared(stream, sizeParserFixed, false);
  }
