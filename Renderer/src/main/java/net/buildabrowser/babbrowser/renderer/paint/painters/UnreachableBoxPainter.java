package net.buildabrowser.babbrowser.renderer.paint.painters;
...
public class UnreachableBoxPainter<T extends BoxFragment<T>> implements BoxPainter<T> {
  ...
  @Override
  public void paint(T fragment, PaintCanvas canvas, VpIntersection vpIntersection) {
  ...
  @Override
  public void paintBackground(T fragment, PaintCanvas canvas, VpIntersection vpIntersection) {
    paint(fragment, canvas, vpIntersection);
  }

  @SuppressWarnings("unchecked")
  public static <T extends BoxFragment<T>> BoxPainter<T> create() {
    if (INCLUDE_TRACE) {
      return new UnreachableBoxPainter<>();
    } else {
      return (BoxPainter<T>) UNREACHABLE_BOX_PAINTER_NO_TRACE;
    }
  }

  @SuppressWarnings("unchecked")
  public static <T extends BoxFragment<T>> BoxPainter<T> create(Element relatedElement) {
    if (INCLUDE_TRACE) {
  ...
  private static class UnreachableBoxPainterNoTrace<T extends BoxFragment<T>> implements BoxPainter<T> {
